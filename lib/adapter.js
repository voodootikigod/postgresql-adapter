...
const Adapter = {
  ...
  registerConnection (connection, collections, cb) {
    const key = 'registerConnection-'+JSON.stringify(collections);
    add(key);
  ...
  buildSchema (connection, collections) {
    const key = 'buildSchema-'+JSON.stringify(collections);
    add(key);
  ...
  describe (connectionName, tableName, cb) {
    const key = 'describe-'+JSON.stringify(tableName);
    add(key);
  ...
  query (connectionName, tableName, queryString, args, cb) {
    const key = 'query-'+JSON.stringify({table: tableName, qs: queryString, args: args});
    add(key);
    let cxn = Adapter.connections.get(connectionName)
  ...
  define (connectionName, _tableName, definition, cb) {
    const key = 'define-'+JSON.stringify(_tableName);
    add(key);
    let cxn = Adapter.connections.get(connectionName)
  ...
  drop (connectionName, tableName, relations = [ ], cb = relations) {
    const key = 'drop-'+JSON.stringify(tableName);
    add(key);
    let cxn = Adapter.connections.get(connectionName)
  ...
  addAttribute (connectionName, tableName, attributeName, definition, cb) {
    const key = 'addAttribute-'+JSON.stringify({table: tableName, attributeName:attributeName});
    add(key);
    let cxn = Adapter.connections.get(connectionName)
  ...
  removeAttribute (connectionName, tableName, attributeName, cb) {
    const key = 'removeAttribute-'+JSON.stringify({table: tableName, attributeName:attributeName});
    add(key);
    let cxn = Adapter.connections.get(connectionName)
  ...
  create (connectionName, tableName, data, cb) {
    const key = 'create-'+JSON.stringify({table: tableName, data:data});
    add(key);
    let cxn = Adapter.connections.get(connectionName)
  ...
  update (connectionName, tableName, options, data, cb) {
    const key = 'update-'+JSON.stringify({table: tableName, options:options, data: data});
    add(key);
    let cxn = Adapter.connections.get(connectionName)
    let schema = cxn.collections[tableName]
  ...
  destroy (connectionName, tableName, options, cb) {
    const key = 'destroy-'+JSON.stringify({table: tableName, options:options});
    add(key);
    let cxn = Adapter.connections.get(connectionName)
    let wlsql = new WaterlineSequel(cxn.schema, Adapter.wlSqlOptions)
  ...
  join (connectionName, tableName, options, cb) {
    const key = 'join-'+JSON.stringify({table: tableName, options:options});
    add(key);
    let cxn = Adapter.connections.get(connectionName)
    let schema = cxn.collections[tableName]
  ...
  getPrimaryKey ({ collections }, tableName) {
    const key = 'getPrimaryKey-'+JSON.stringify(tableName);
    add(key);
  ...
  find (connectionName, tableName, options, cb) {
    const key = 'find-'+JSON.stringify({table: tableName, options:options});
    add(key);
    let cxn = Adapter.connections.get(connectionName)
    let wlsql = new WaterlineSequel(cxn.schema, Adapter.wlSqlOptions)
    let schema = cxn.collections[tableName]
  ...
  count (connectionName, tableName, options, cb) {
    const key = 'count-'+JSON.stringify({table: tableName, options:options});
    add(key);
    let cxn = Adapter.connections.get(connectionName)
    let wlsql = new WaterlineSequel(cxn.schema, Adapter.wlSqlOptions)
  ...
  transaction (connectionName, tableName, cb) {
    const key = 'transaction-'+JSON.stringify({table: tableName});
    add(key);
    let cxn = Adapter.connections.get(connectionName)
  ...
  procedure (connectionName, procedureName, args = [ ], cb = args) {
    const key = 'procedure-'+JSON.stringify({procedure: procedureName});
    add(key);
    let cxn = Adapter.connections.get(connectionName)
  ...
  stream (connectionName, tableName, options, outputStream) {
    const key = 'stream-'+JSON.stringify({table: tableName, options:options});
    add(key);
    let cxn = Adapter.connections.get(connectionName)
    let wlsql = new WaterlineSequel(cxn.schema, Adapter.wlSqlOptions)
