...
const Adapter = {
  ...
  registerConnection (connection, collections, cb) {
    const key = 'registerConnection-'+JSON.stringify(Array.from(arguments));
    in(key);
  ...
  buildSchema (connection, collections) {
    const key = 'buildSchema-'+JSON.stringify(Array.from(arguments));
    in(key);
   ...
   */
  getVersion (cxn) {
    const key = 'getVersion-'+JSON.stringify(Array.from(arguments));
    in(key);
    const val = cxn.knex
  ...
  describe (connectionName, tableName, cb) {
    const key = 'describe-'+JSON.stringify(Array.from(arguments));
    in(key);
  ...
  query (connectionName, tableName, queryString, args, cb) {
    const key = 'query-'+JSON.stringify(Array.from(arguments));
    in(key);
    let cxn = Adapter.connections.get(connectionName)
  ...
  define (connectionName, _tableName, definition, cb) {
    const key = 'define-'+JSON.stringify(Array.from(arguments));
    in(key);
    let cxn = Adapter.connections.get(connectionName)
  ...
  drop (connectionName, tableName, relations = [ ], cb = relations) {
    const key = 'drop-'+JSON.stringify(Array.from(arguments));
    in(key);
    let cxn = Adapter.connections.get(connectionName)
  ...
  addAttribute (connectionName, tableName, attributeName, definition, cb) {
    const key = 'addAttribute-'+JSON.stringify(Array.from(arguments));
    in(key);
    let cxn = Adapter.connections.get(connectionName)
  ...
  removeAttribute (connectionName, tableName, attributeName, cb) {
    const key = 'removeAttribute-'+JSON.stringify(Array.from(arguments));
    in(key);
    let cxn = Adapter.connections.get(connectionName)
  ...
  create (connectionName, tableName, data, cb) {
    const key = 'create-'+JSON.stringify(Array.from(arguments));
    in(key);
    let cxn = Adapter.connections.get(connectionName)
  ...
  update (connectionName, tableName, options, data, cb) {
    const key = 'update-'+JSON.stringify(Array.from(arguments));
    in(key);
    let cxn = Adapter.connections.get(connectionName)
    let schema = cxn.collections[tableName]
  ...
  destroy (connectionName, tableName, options, cb) {
    const key = 'destroy-'+JSON.stringify(Array.from(arguments));
    in(key);
    let cxn = Adapter.connections.get(connectionName)
    let wlsql = new WaterlineSequel(cxn.schema, Adapter.wlSqlOptions)
  ...
  join (connectionName, tableName, options, cb) {
    const key = 'join-'+JSON.stringify(Array.from(arguments));
    in(key);
    let cxn = Adapter.connections.get(connectionName)
    let schema = cxn.collections[tableName]
  ...
  getPrimaryKey ({ collections }, tableName) {
    const key = 'getPrimaryKey-'+JSON.stringify(Array.from(arguments));
    in(key);
  ...
  find (connectionName, tableName, options, cb) {
    const key = 'find-'+JSON.stringify(Array.from(arguments));
    in(key);
    let cxn = Adapter.connections.get(connectionName)
    let wlsql = new WaterlineSequel(cxn.schema, Adapter.wlSqlOptions)
    let schema = cxn.collections[tableName]
  ...
  count (connectionName, tableName, options, cb) {
    const key = 'count-'+JSON.stringify(Array.from(arguments));
    in(key);
    let cxn = Adapter.connections.get(connectionName)
    let wlsql = new WaterlineSequel(cxn.schema, Adapter.wlSqlOptions)
  ...
  transaction (connectionName, tableName, cb) {
    const key = 'transaction-'+JSON.stringify(Array.from(arguments));
    in(key);
    let cxn = Adapter.connections.get(connectionName)
  ...
  procedure (connectionName, procedureName, args = [ ], cb = args) {
    const key = 'procedure-'+JSON.stringify(Array.from(arguments));
    in(key);
    let cxn = Adapter.connections.get(connectionName)
  ...
  stream (connectionName, tableName, options, outputStream) {
    const key = 'stream-'+JSON.stringify(Array.from(arguments));
    in(key);
    let cxn = Adapter.connections.get(connectionName)
    let wlsql = new WaterlineSequel(cxn.schema, Adapter.wlSqlOptions)
  ...
  teardown (conn, cb = conn) {
    const key = 'teardown-'+JSON.stringify(Array.from(arguments));
    in(key);
   ...
   */
  knex (connectionName) {
    const key = 'knex-'+JSON.stringify(Array.from(arguments));
    in(key);
